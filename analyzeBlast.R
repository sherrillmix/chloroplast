...
badSamples<-rownames(propId)[propId[,'Eukaryota']<.2]
...
rares<-lapply(unique(baseName),function(xx){
  ...
  return(rareEquation(table(thisTaxas),round(seq(.005,1,.005)*length(thisTaxas))))
})
...
for(primer in unique(primers)){
  thisRares<-rares[primers==primer&!names(rares) %in% sub('_.*','',badSamples)]
  plot(1,1,type='n',ylab='Number of taxa identified',xlab='',ylim=ylim,las=1,xlim=xlim/100000,main=primer,mgp=c(3,1,0))
...
rares<-lapply(unique(baseName),function(xx){
  thisTaxas<-unlist(lapply(taxas[baseName==xx],function(yy)ifelse(is.na(yy$species),'Unknown',yy$species)))
  return(rareEquation(table(thisTaxas),round(seq(.005,1,.005)*length(thisTaxas))))
})
names(rares)<-unique(baseName)
primers<-sub('KSG[0-9]+','',sub('_.*','',names(rares)))
xlim<-c(0,max(as.numeric(unlist(lapply(rares,names)))))
ylim<-c(1,max(as.numeric(unlist(rares))))
pdf('out/rarefactionsSpecies.pdf')
par(mfrow=c(2,2),mar=c(4,4.3,1,.3))
for(primer in unique(primers)){
  thisRares<-rares[primers==primer&!names(rares) %in% sub('_.*','',badSamples)]
  plot(1,1,type='n',ylab='Number of species identified',xlab='',ylim=ylim,las=1,xlim=xlim/100000,main=primer,mgp=c(3,1,0))
  title(xlab='Reads sequenced (100,000)',mgp=c(2,1,0))
  sapply(thisRares,function(x)lines(as.numeric(names(x))/100000,x,col='#00000077',lwd=2))
}
dev.off()
...
selectOtu<-selectOtu[,!colnames(selectOtu) %in% sub('_R[12]_','_',badSamples)]
